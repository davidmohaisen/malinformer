...
def topFeatOccurrence(args):
    ...
    for family in familyList:
        ...
        with open(direc + "discriminativeFeatsOccurrences/noThreshold/" + family + "-" + str(itm) + '-topFeatsOccurrences.pickle', 'wb') as handle:
...
def main():
    ...
    for family in fam_count:
        ...
        for itm in lengthArray:
            ...
            indices = np.where(np.isin(overallFeats, famTopFeats_module))[0]
            print(family, ":", len(famTopFeats_module), ":", len(indices))
    ...
    Arguments = []
    for itm in itm_FeatIndices:
        Arguments.append([direc, itm_FeatIndices[itm], familyList, itm])
    ...
    with open(direc + "discriminativeFeatsOccurrences/noThreshold/" + 'familyList.pickle', 'wb') as handle:
